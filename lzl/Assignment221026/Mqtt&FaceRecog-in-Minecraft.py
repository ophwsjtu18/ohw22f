...
def face2direction(img):
    ...
    if len(faces) != 0:
        for (x, y, w, h) in faces:
            ...
            mid = x + w / 2
            if mid < width / 3:
                cv2.putText(img, "left", (x, y), cv2.FONT_HERSHEY_SCRIPT_SIMPLEX, 1, (0, 255, 0), 2)
                return LEFT
            elif mid > 2 * width / 3:
                cv2.putText(img, "right", (x, y), cv2.FONT_HERSHEY_SCRIPT_SIMPLEX, 1, (0, 255, 0), 2)
                return RIGHT
            elif w > width / 4:
                cv2.putText(img, "top", (x, y), cv2.FONT_HERSHEY_SCRIPT_SIMPLEX, 1, (0, 255, 0), 2)
                return FORWARD
            elif w < width / 6:
                cv2.putText(img, "down", (x, y), cv2.FONT_HERSHEY_SCRIPT_SIMPLEX, 1, (0, 255, 0), 2)
                return BACKWARD
            else:
                cv2.putText(img, "standby", (x, y), cv2.FONT_HERSHEY_SCRIPT_SIMPLEX, 1, (0, 255, 0), 2)
                return STANDBY
