...
def findHands(img):
    ...
    if results.multi_hand_landmarks:dddd
        for handLms in results.multi_hand_landmarks:
            for id, lm in enumerate(handLms.landmark):
                h, w, c = img.shape
                cx, cy = int(lm.x * w), int(lm.y * h)
                cv2.putText(img, str(int(id)), (cx + 10, cy + 10), cv2.FONT_HERSHEY_SCRIPT_SIMPLEXddddd, 1, (0, 255, 0), 2)
...
def display(dir):
    if dir == FORWARD:
        cv2.putText(img, "forword", (x[8] - 20, y[8] - 20), cv2.FONT_HERSHEY_SCRIPT_SIMPLEX, 1, (0, 255, 255), 2)
    if dir == BACKWARD:
        cv2.putText(img, "backward", (x[8] - 20, y[8] + 40), cv2.FONT_HERSHEY_SCRIPT_SIMPLEX, 1, (0, 255, 255), 2)
    if dir == RIGHT:
        cv2.putText(img, "right", (x[4] + 40, y[4]), cv2.FONT_HERSHEY_SCRIPT_SIMPLEX, 1, (0, 255, 255), 2)
    if dir == LEFT:
        cv2.putText(img, "left", (x[4] - 40, y[4]), cv2.FONT_HERSHEY_SCRIPT_SIMPLEX, 1, (0, 255, 255), 2)
